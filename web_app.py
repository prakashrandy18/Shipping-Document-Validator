...
@app.route('/youtube')
def youtube_page():
    """Render the YouTube player page."""
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            for e in entries:
                ...
                videos.append({
                    ...
                    'thumbnail': f"/youtube/thumb/{e.get('id', '')}",
    ...
    try:
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:
            ...
            return jsonify({
                ...
                'thumbnail': f"/youtube/thumb/{video_id}",
...
@app.route('/youtube/thumb/<video_id>')
def youtube_thumb(video_id):
    """Proxy YouTube thumbnails so they load on restricted networks."""
    try:
        thumb_url = f"https://i.ytimg.com/vi/{video_id}/hqdefault.jpg"
        resp = http_requests.get(thumb_url, timeout=10, stream=True)
        return Response(
        ...
        return Response(b'', content_type='image/jpeg', status=404)


@app.route('/youtube/stream')
def youtube_stream():
    """Download and serve a YouTube video via pytubefix."""
    ...
    # Check cache first
    cache_dir = os.path.join(tempfile.gettempdir(), 'yt_cache')
    os.makedirs(cache_dir, exist_ok=True)
    cached_file = os.path.join(cache_dir, f'{video_id}.mp4')

    if not os.path.exists(cached_file):
        try:
            url = f"https://www.youtube.com/watch?v={video_id}"
            logger.info(f"Downloading video {video_id} via pytubefix...")
            yt = PyTube(url)
            ...
            if not stream:
                return jsonify({'error': 'No suitable stream found'}), 500
            
            stream.download(output_path=cache_dir, filename=f'{video_id}.mp4')
            logger.info(f"Download complete: {video_id} ({os.path.getsize(cached_file)} bytes)")
        except Exception as e:
    ...
    if range_header:
        import re as _re2
        match = _re2.search(r'bytes=(\d+)-(\d*)', range_header)
        if match:
    ...
    return send_file(cached_file, mimetype='video/mp4', conditional=True)
